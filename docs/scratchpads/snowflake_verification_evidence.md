...
| V7 purchase/receipt candidates | `BASE_INVENTORY`: 106 lines, 35 POs, all closed and fully delivered, latest sync 2025-10-20; all 106 nonblank delivery strings fail `TRY_TO_DATE`. `BASE_STOCKS`: 21 rows, one location, latest sync 2025-10-30; every supplier article number is missing. Joel confirmed no PO data is currently ingested into Snowflake to his knowledge. | Neither table can support current open-PO netting; the missing feed is confirmed rather than merely undiscovered in the reviewed schemas. | Identify the current Ops system/process with Deepali/Dor/Ilona, then coordinate ingestion and a normalized PO/receipt model with Joel. |
...
| V3 silo/BOM join | 62 ingredient IDs are ambiguous under an ingredient-only join. In the first active-menu/chamber test, 2,576 stock keys produced 1,099 without same-day menu context, 1,477 with menu context but no tested ingredient-slot match, and zero resolved slots. The accompanying raw sample shows `CHAMBER_ALIAS` is the same literal `Chamber Alias` on all 100 rows, while `SILO_RESOURCE_ID` carries values such as `1`, `7`, and `51`. | The zero matches do **not** prove no mapping exists: the first query compared the constant alias rather than the numeric resource and tested only one detailed-menu ingredient field. Ingredient-only capacity remains invalid. | Run corrected V3B, which tests active-menu matches across days, alternative ingredient identifiers, and `SILO_RESOURCE_ID ↔ RECIPE_SLOT_INSERTING_POSITION`. Ask for lineage only if that still cannot resolve a stable slot. |
...
## Exports or diagnostics still needed

1. Corrected V3B active-menu/resource/ingredient-to-recipe-slot join coverage.
...
- The abandoned forecasts/recommendations are reference evidence only. The
  flattened/versioned BOM remains a credible adapter candidate pending lineage
  and physical-slot resolution.
