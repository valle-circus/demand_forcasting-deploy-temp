...
- **Evidence received 2026-08-25:** V1-V12 and all saved follow-ups are summarized in `docs/scratchpads/snowflake_verification_evidence.md`; no required verification query remains. Joel confirmed that `FACT_CG_DISH_DEMAND_FORECASTS`, `FACT_CG_INGREDIENT_DEMAND_FORECASTS`, `FACT_CG_PURCHASE_ORDERS`, and `BASE_INVENTORY` are abandoned previous-data-team models. They may be replaced in [`data-transformation`](https://github.com/circus-kitchens/data-transformation), but are not operational sources of truth. Joel will provision a stable RSA-authenticated Snowflake service account through 1Password. V5 measures 76 item rows/75 IDs with complete pack/unit values, seven missing EANs, ten missing Apicbase IDs, and one blank ID. V11 proves the materialized menu ended on 2026-08-24 when queried on 2026-08-25. V1/V2B closes the saved location/per-unit sales checks at 622 portions. Corrected V3B found 1,636/2,576 stock keys with menu context through `INGREDIENT_KEY`, but zero resource-position or exact-slot matches, ruling out the tested direct position join.
- **Actions now:**
  ...
  4. Inspect `data-transformation` and upstream lineage for the authoritative unit/resource/dock-to-effective-recipe-slot bridge. If it is absent, ask Joel or the robot/menu data owner; do not infer one from ingredient identity. Enhanced V12 remains optional until calibration is in scope.
...
- **Physical-slot follow-up:** V3B could not settle physical identity and rejected the tested direct join. Resolve this through repository/upstream lineage or a targeted owner answer before using physical silo capacity. Forward-menu commitment remains a separate business/source question because the materialized table has no forward coverage.
- **Fallback until then:** canonical file adapters with explicit provenance. A reviewed manual PO snapshot can exercise strict source gates, but it is not silently treated as a complete live pipeline or sufficient for production approval; unknown placeholders are limited to fixture/scenario mode.
...
- **Fallback until then:** the pure event ledger, dated netting, and manual `open_pos.csv` adapter are implemented. Continue policy-engine work with synthetic or reviewed manual snapshots. Fixture/scenario mode may explicitly use an unknown empty pipeline; shadow/operational mode refuses it before emitting netting results.
